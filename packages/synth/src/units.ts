...
interface UnitMeta<TUnit extends UnitName> {
  readonly range: Range
  readonly min: UnitValue<TUnit>
  readonly max: UnitValue<TUnit>
  (x: number): UnitValue<TUnit>
...
const createUnitMeta = <TUnit extends UnitName>(unitName: TUnit, range: Range): UnitMeta<TUnit> => {
  const is = (x: number): x is UnitValue<TUnit> => range.includes(x)

  const orThrow = (x: number) => {
    if (!is(x)) {
      throw new Error(`the argument is not a valid ${unitName} value in range ${range}`)
    }

    return x
  }

  const orClamp = (x: number) => range.clamp(x) as UnitValue<TUnit>
